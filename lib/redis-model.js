...
RedisModel.prototype.uniqId = function (callback) {
  this.db.incr(this.kCounter(), function (err, reply) {
    var hash = base58.encode(getRandomInt(9999, 99999999) + reply.toString());
...
RedisModel.prototype.getAnalytics = function (hash,callback) {
  var self = this;
  this.db.keys(RedisModel._prefix_+"hash:"+hash,function(err,res){
    if(err){
      callback(err);
    }
    else{
      async.mapLimit(res,10,function(r,cb){
        self.db.hgetall(r,function(err,res){
          cb(err,res);
        });
      },callback);
    }    
